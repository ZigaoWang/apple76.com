...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!file || typeof file === 'string') {
      return NextResponse.redirect(`${baseUrl}/admin?error=nofile`);
    }
    ...
    return NextResponse.redirect(`${baseUrl}/admin?success=upload`);
  } catch (e) {
    console.error('Upload error:', e);
    return NextResponse.redirect(`${baseUrl}/admin?error=upload`);
  }
} 